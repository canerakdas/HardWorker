...
class HardWorker {
  ...
  */
  ...
  setConfig() {
    ...
    // Checking config file has multipler, if doesnt exist appy to default 1.
    this.config.multipler = this.config.multipler || 1;
    ...
    // Floor(Thread Count * Multipler)
    this.config.threadCount = Math.floor(this.config.navigatorThreadCount * this.config.multipler);
